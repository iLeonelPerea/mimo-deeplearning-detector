...
def generate_training_data(N, symbol_combinations, double_one_hot, SNR_linear, No, Nr, Nt, device='cpu', use_zf=False, decouple_antennas=False, channel_mode='fixed', snr_mode='variable', fixed_snr_db=3):
...
X_data, y_labels, random_indices, H_training = generate_training_data(
    N_TRAINING_SAMPLES, symbol_combinations, double_one_hot, SNR_linear, No, Nr, Nt, device,
...
model = MIMO_Detector_DoubleOneHot(
    ...
    HIDDEN_SIZE,
    output_size,
    use_sigmoid_hidden=True,  # One-Hot Per Antenna uses Sigmoid + ReLU
    use_bias=USE_BIAS
).to(device)
print("="*70)
...
plt.tight_layout()
plt.savefig('training_DoubleOneHot_curves.png', dpi=300, bbox_inches='tight')
plt.close()
print("✓ Training curves plot saved to: training_DoubleOneHot_curves.png")
